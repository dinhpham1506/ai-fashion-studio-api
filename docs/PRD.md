...
## 4. Main flow
Guest → xem About Us/Product/Feedback → Register/Login → Browse → chọn variant → Design Editor → Save Design → Add to Cart → Checkout/Create Order → Payment → Order PAID (lock design) → Staff cập nhật production/shipping → Order Completed → Feedback.
...
- BR-006 không add cart / tạo order nếu variant hết hàng.
...
## 9. Database
20 bảng / 8 schema trong 1 PostgreSQL. Chi tiết: [DATABASE_DESIGN.md](DATABASE_DESIGN.md).
